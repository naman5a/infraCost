...
import { CostComponent, HOURS_PER_MONTH, TfResource } from "./types.js";

type Mapper = (r: TfResource) => CostComponent[];
...
const SHARED_CORE: Record<string, { cpu: number; ramGb: number }> = {
  "e2-micro": { cpu: 0.25, ramGb: 1 },
  "e2-small": { cpu: 0.5, ramGb: 2 },
  "e2-medium": { cpu: 1, ramGb: 4 },
};

const RAM_PER_CPU: Record<string, number> = {
  standard: 4,
  highmem: 8,
  highcpu: 1,
};
...
export function machineSpecs(machineType: string): MachineSpecs | null {
  const mt = machineType.toLowerCase();
  if (SHARED_CORE[mt]) return { ...SHARED_CORE[mt], family: mt.split("-")[0] };
  ...
  if (parts.length === 3) {
    ...
    if (!isNaN(cpu) && perCpu !== undefined) {
      ...
      const ramGb =
        family === "n1"
          ? cpu * ({ standard: 3.75, highmem: 6.5, highcpu: 0.9 } as any)[kind]
          : cpu * perCpu;
...
const FAMILY_DESC: Record<string, { core: string; ram: string }> = {
...
const DISK_DESC: Record<string, string> = {
...
function diskComponent(diskType: string, sizeGb: number, region: string, label: string): CostComponent {
...
const computeInstance: Mapper = (r) => {
  const v = r.values;
...
const STORAGE_CLASS_DESC: Record<string, string> = {
...
const sqlDatabaseInstance: Mapper = (r) => {
  const v = r.values;
  const settings = v.settings?.[0] ?? {};
  const ver: string = v.database_version ?? "POSTGRES_15";
...
const containerCluster: Mapper = (r) => {
  const v = r.values;
  ...
  if (!v.remove_default_node_pool) {
    ...
    components.push(
      ...vmComponents(mt, r.region, false, nodes, ` × ${nodes} default nodes`),
      diskComponent(
        v.node_config?.[0]?.disk_type ?? "pd-balanced",
        (v.node_config?.[0]?.disk_size_gb ?? 100) * nodes,
        r.region,
        `Node boot disks × ${nodes}`
      )
    );
  }
  return components;
...
const containerNodePool: Mapper = (r) => {
  const v = r.values;

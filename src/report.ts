...
import { BreakdownResult } from "./types.js";

const money = (n: number) => `$${n.toFixed(2)}`;
const price = (n?: number) => (n === undefined ? "-" : `$${n.toFixed(n < 0.01 ? 6 : 4)}`);
const qty = (n: number) => (Number.isInteger(n) ? String(n) : n.toFixed(2));

export function renderTable(result: BreakdownResult): string {
  const rows: string[][] = [];
  ...
  for (const row of rows.slice(1)) out.push(line(row).trimEnd());
  out.push("─".repeat(widths.reduce((a, b) => a + b + 2, -2)));
  ...
  if (unsup.length > 0) {
    ...
    for (const [type, count] of unsup) out.push(`  - ${type} × ${count}`);
  }
  out.push("");
...
export function renderJson(result: BreakdownResult): string {
